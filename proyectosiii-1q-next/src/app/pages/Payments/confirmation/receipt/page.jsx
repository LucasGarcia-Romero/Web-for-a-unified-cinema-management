...
export default function Receipt() {
  ...
  const quantity = paymentData.selectedSeats?.length || 1;
  const originalPrice = paymentData.price || 0;
  const discountedPrice = calculateDiscountedPrice(originalPrice, quantity, discountType);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#343b59] flex flex-col items-center justify-center p-6">
        <div className="flex gap-4 mb-4 no-print">
          <button 
            onClick={handlePrint}
            className="px-6 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors duration-200 shadow-lg"
          >
            Save as PDF / Print
          </button>
        </div>
        ...
        <div
          id="receipt-content"
          ref={receiptRef}
          className="p-6 w-full max-w-[400px] font-mono text-sm leading-relaxed text-black bg-gradient-to-b from-white to-gray-100 rounded-lg shadow-xl"
        >
          ...
          <div className="border-t border-b border-black py-4 mb-6">
            ...
            {snacksCart.map((snack, index) => (
              <div key={index} className="flex text-black text-lg">
                <div className="w-16">{snack.quantity}</div>
